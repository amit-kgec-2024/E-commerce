...
// Management register.....................
router.post("/management/register", async (req, res, next) => {
  try {
    const { username, email, password } = req.body;
    if (!username || !email || !password) {
      res.status(200).send("Plese all require files");
    } else {
      const isAlreadyExist = await Management.findOne({ email });
      if (isAlreadyExist) {
        res.status(400).send("User alredy Exist");
      } else {
        const newUser = new Management({ username, email });
