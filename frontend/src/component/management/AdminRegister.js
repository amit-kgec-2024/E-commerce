  ...
  return (
    <div className="flex h-screen flex-row justify-around items-center bg-gray-100">
        <div className="border rounded shadow p-4 flex flex-col items-center">
            ...
            <h2 className='text-md font-extralight uppercase mb-8'>ples register to Admin</h2>
            <form className="flex flex-col items-center" onSubmit={(e)=> handelSubmit(e)}>
                <Input type='text' label='First Name' placeholder='firstname..' value={data.firstname} onChange={(e)=> setData({...data, firstname: e.target.value})}/>
                <Input type='text' label='Last Name' placeholder='lastname..' value={data.lastname} onChange={(e)=> setData({...data, lastname: e.target.value})}/>
