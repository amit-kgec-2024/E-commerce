...
import { optionStars, optionCategoryFashion, optionSale } from "../../utils/dropdown";
...
const AdminFashion = () => {
  ...
  const handelSubmit = async (e) => {
    ...
    const res = await fetch(
      "https://e-commerce-nu-seven.vercel.app/api/fashion/register",
      {
        method: "POST",
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `https://e-commerce-nu-seven.vercel.app/api/fashion/delete/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  <div>
    <div className="w-full bg-stone-500 h-12 flex flex-row justify-between items-center">
      <h1 className="font-bold text-sm md:text-xl text-white px-6">Fashions</h1>
    </div>
      ...
      <div className="border shadow flex p-3 items-center w-full sm:w-[55%] md:w-[40%] lg:w-[30%]">
        ...
        >
          ...
          >
            ...
            <select className="form-select outline-none border">
              {optionCategoryFashion.map((option) => (
                <option value={option.value}>{option.label}</option>
              ))}
            </select>
          </div>
      ...
      <div className="border shadow flex flex-col p-3 items-center w-full sm:w-[55%] md:w-[40%] lg:w-[30%] h-[600px] overflow-y-scroll">
        {getData.map((e, index) => (
          ...
          >
            ...
            <button onClick={()=> handleDelete(e.product.id)} className="px-3 py-1 bg-emerald-100 text-red-400 rounded-lg">Remove</button>
            </div>
            <img
              src={e.product.img}
              alt={e.product.title}
