...
import { optionStars, optionCategoryKitchen, optionSale } from "../../utils/dropdown";
...
const AdminKitchen = () => {
  ...
  const handelSubmit = async (e) => {
    ...
    const res = await fetch(
      "https://e-commerce-nu-seven.vercel.app/api/kitchen/register",
      {
        method: "POST",
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `https://e-commerce-nu-seven.vercel.app/api/kitchen/delete/${id}`,
        {
          method: "DELETE",
        }
      );
        ...
        <div className="border shadow flex p-3 items-center w-full sm:w-[55%] md:w-[40%] lg:w-[30%]">
          ...
          >
            ...
            >
              ...
              <select className="form-select outline-none border">
                {optionCategoryKitchen.map((option) => (
                  <option value={option.value}>{option.label}</option>
                ))}
              </select>
            </div>
