...
const AdminElectronics = () => {
  ...
  const handelSubmit = async (e) => {
    ...
    const res = await fetch(
      "https://e-commerce-nu-seven.vercel.app/api/electronics/register",
      {
        method: "POST",
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `https://e-commerce-nu-seven.vercel.app/api/electronics/delete/${id}`,
        {
          method: "DELETE",
        }
      );
        ...
        <div className="border shadow flex p-3 items-center w-full sm:w-[55%] md:w-[40%] lg:w-[30%]">
          ...
          >
            ...
            >
              ...
              <select className="form-select outline-none border">
                {optionCategoryElectronics.map((option) => (
                  <option value={option.value}>{option.label}</option>
                ))}
              </select>
            </div>
